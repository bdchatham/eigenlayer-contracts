...
interface IReleaseManager is IReleaseManagerErrors, IReleaseManagerEvents {
    struct PublishedRelease {
        bytes32 digest;              // OCI artifact digest
        string registryUrl;          // Full OCI registry URL
        string version;              // Semantic version
        uint256 deploymentDeadline;  // Deployment deadline timestamp
        uint256 publishedAt;         // Publication timestamp
    }
