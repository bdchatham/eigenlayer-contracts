...
contract ReleaseManager is ReentrancyGuard, OwnableUpgradeable, ReleaseManagerStorage {
    ...
    function publishRelease(
        ...
        Checkpoints.push(releaseIndexHistory[avs], uint32(block.number), uint224(newIndex));
    ...
    function getReleaseAtBlock(
        ...
        uint256 index = Checkpoints.upperLookup(releaseIndexHistory[avs], uint32(blockNumber));
        PublishedRelease[] memory releases = allPublishedReleases[avs];
    ...
    function getReleaseCheckpointCount(
        ...
        return Checkpoints.length(releaseIndexHistory[avs]);
    }
}